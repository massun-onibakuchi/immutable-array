...
contract DynamicSizeArray3ExampleTest is ExampleBaseTest {
    function setUp() public {
        uint256 _before = gasleft();
        example = IArrayExample(
            address(new DynamicSizeArrayExample(testArray))
        );
        console2.log("gas usage: ", _before - gasleft());
    }
}
...
contract StaticSizeArray3ExampleTest is ExampleBaseTest {
    function setUp() public {
        ...
        uint256 _before = gasleft();
        example = IArrayExample(
            address(new StaticSizeArray3Example(_testArray))
        );
        console2.log("gas usage: ", _before - gasleft());
    }

    function test_getArray() public override {
        address[3] memory result = StaticSizeArray3Example(address(example))
            .getArray();
        for (uint256 i = 0; i < testArray.length; i++) {
            assertEq(result[i], testArray[i]);
        }
...
contract StaticSizeArray6ExampleTest is ExampleBaseTest {
    ...
    function setUp() public {
        ...
        uint256 _before = gasleft();
        example = IArrayExample(
            address(new StaticSizeArray6Example(_testArray))
        );
        console2.log("gas usage: ", _before - gasleft());
    }

    function test_getArray() public override {
        address[6] memory result = StaticSizeArray6Example(address(example))
            .getArray();
        for (uint256 i = 0; i < testArray.length; i++) {
            assertEq(result[i], testArray[i]);
        }
...
contract ImmutableValue3ExampleTest is ExampleBaseTest {
    function setUp() public {
        ...
        uint256 _before = gasleft();
        example = IArrayExample(
            address(new ImmutableValue3Example(_testArray))
        );
        console2.log("gas usage: ", _before - gasleft());
    }

    function test_getArray() public override {
        address[3] memory result = ImmutableValue3Example(address(example))
            .getArray();
        for (uint256 i = 0; i < testArray.length; i++) {
            assertEq(result[i], testArray[i]);
        }
